...
- 请求支持 `tenant_id`、`user_id`、`session_id`、`message`、`images` 和受限内联文本 `files`。
...
高级区域允许查看完整 JSON；结构化字段仍作为默认入口。附件选择器支持图片和文本文件，图片转换为 Base64 `images`，文本文件转换为 `files[].content`。
...
页面只保存最近 20 次请求，保存在内存中，不落库 API Key。历史项保留当前页面内的图片/文本引用，以便使用当前 Key 重放；刷新页面后全部清空。
...
```json
{
  ...
  "files": [{"name": "context.md", "media_type": "text/markdown", "content": "补充背景"}]
}
```

`files` 只接受文本媒体类型，最多 8 个、单个 200,000 字符、合计 500,000 字符。二进制文档不走内联 Prompt，应使用平台文档上传和解析链路。
...
2. 二进制文档引用何时扩展到外部 API（图片与内联文本已经支持）。
