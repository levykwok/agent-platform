...
const form = reactive({ task_id: '', name: '', agent_id: '', session_id: '', prompt: '', cron: '0 */5 * * * *', timezone: '' })

function resetForm() {
  Object.assign(form, { task_id: '', name: '', agent_id: '', session_id: '', prompt: '', cron: '0 */5 * * * *', timezone: '' })
  selected.value = null
  runs.value = []
}
...
async function save() {
  ...
  try {
    ...
    const data = await readJson<Row>(await fetch(path, { method, headers: makeHeaders(true), body: JSON.stringify(form) }))
