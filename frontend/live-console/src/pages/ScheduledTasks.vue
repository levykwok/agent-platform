...
const form = reactive({ task_id: '', name: '', target_type: 'agent', agent_id: '', workflow_id: '', session_id: '', prompt: '', cron: '0 */5 * * * *', timezone: '', webhook_url: '', webhook_secret: '', webhook_enabled: true })

function resetForm() {
  Object.assign(form, { task_id: '', name: '', target_type: 'agent', agent_id: '', workflow_id: '', session_id: '', prompt: '', cron: '0 */5 * * * *', timezone: '', webhook_url: '', webhook_secret: '', webhook_enabled: true })
  selected.value = null
  runs.value = []
}
...
function edit(task: Row) {
  Object.assign(form, {
    task_id: task.task_id || '', name: task.name || '', target_type: task.target_type || 'agent', agent_id: task.target_type === 'workflow' ? '' : (task.target_id || task.agent_id || ''), workflow_id: task.workflow_id || '', session_id: task.session_id || '',
...
async function save() {
  ...
  try {
    ...
    if (form.target_type === 'workflow') delete payload.agent_id
...
<template>
  <section class="scheduled-page">
    <div class="scheduled-head">
      <div><div class="eyebrow">SCHEDULED TASKS</div><h1>定时任务</h1><p>按 Cron 定时唤起 Agent 或固定版本 Workflow，结果写入绑定会话并生成通知。</p></div>
    ...
    <div class="scheduled-grid">
      <div class="scheduled-panel task-list-panel">
        ...
        <button v-for="task in tasks" :key="task.task_id" class="task-item" :class="{selected: selected?.task_id === task.task_id}" @click="edit(task)">
          ...
          <span class="task-item-main"><strong>{{ task.name }}</strong><small>{{ task.target_type === 'workflow' ? `Workflow ${task.workflow_id}@v${task.workflow_version}` : task.agent_id }} · {{ task.cron_expression }}</small><small>下次：{{ fmtDate(task.next_run_at) || '—' }}</small></span>
        </button>
      </div>
      <div class="scheduled-panel form-panel">
        ...
        <div class="field-grid">
          ...
          <label>目标类型<select v-model="form.target_type"><option value="agent">Agent</option><option value="workflow">Workflow</option></select></label>
          <label v-if="form.target_type === 'agent'">Agent ID<input v-model="form.agent_id" placeholder="已发布 Agent ID" /></label>
          <label v-else>Workflow ID<input v-model="form.workflow_id" placeholder="已发布 Workflow ID" /><small>创建时固定当前活动版本</small></label>
          <label class="wide">运行输入<textarea v-model="form.prompt" rows="5" placeholder="到点后发送给目标的输入"></textarea></label>
