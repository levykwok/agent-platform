...
function finish(status: 'success' | 'error' | 'aborted') {
  ...
  requestHistory.value.unshift({
    ...
    files: [...files.value],
    status,
    totalMs: startedAt.value && finishedAt.value ? finishedAt.value - startedAt.value : 0,
...
function replayHistory(item: RequestHistory) {
  agentId.value = item.agentId
  callMode.value = item.callMode
    ...
    <div class="external-grid">
      <aside class="external-panel access-panel">
        ...
        <div class="history-section">
          <div class="discovery-head"><span>本页调用历史</span><strong>{{ requestHistory.length }}/20</strong></div>
          <div v-if="requestHistory.length" class="history-list">
            <button v-for="item in requestHistory" :key="item.id" class="history-item" :title="item.message" :disabled="sending" @click="replayHistory(item)">
              <span class="history-status" :class="item.status"></span>
              <span><strong>{{ item.agentId }} · {{ item.callMode === 'stream' ? 'SSE' : 'JSON' }}</strong><small>{{ item.calledAt }} · {{ item.totalMs }} ms · {{ item.message.slice(0, 28) }}</small></span>
            </button>
          </div>
          <div v-else class="history-empty">调用后可从这里使用当前 Key 一键重放</div>
        </div>
      </aside>
      ...
      <section class="external-panel response-panel">
        ...
        <div class="answer-box"><div class="answer-head"><span>Agent 输出</span><div><button :disabled="!responseAnswer && !responseRaw" @click="copyResponse">复制</button><button :disabled="!responseAnswer && !responseRaw" @click="downloadResponse">下载</button><span v-if="callMode === 'stream'" class="stream-label">LIVE STREAM</span></div></div><div v-if="responseAnswer" class="answer-text">{{ responseAnswer }}</div><div v-else class="answer-empty">发送请求后，Agent 的最终输出会显示在这里。</div></div>
...
.history-section{border-top:1px solid var(--border);margin-top:14px;padding-top:12px}.history-list{display:grid;gap:5px;max-height:220px;overflow:auto}.history-item{display:flex;align-items:center;gap:8px;width:100%;padding:7px;border:1px solid var(--border);border-radius:8px;background:#fff;text-align:left;color:var(--text)}.history-item:hover{background:#f8fafc}.history-item>span:nth-child(2){min-width:0}.history-item strong,.history-item small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.history-item strong{font-size:10px}.history-item small{font-size:9px;color:#94a3b8;margin-top:3px}.history-status{width:7px;height:7px;flex:0 0 7px;border-radius:50%;background:#f59e0b}.history-status.success{background:#22c55e}.history-status.error{background:#ef4444}.history-empty{padding:10px;border-radius:8px;background:#f8fafc;color:#94a3b8;text-align:center;font-size:10px;line-height:1.5}
