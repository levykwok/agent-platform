...
在实测中，Agent 通过 `grep_files` 搜索 `agent-platform` 项目目录，能够间接读到历史 `agent_state.json`、workspace 文件、源码示例和 prompt 片段，从而回答跨会话问题。这说明文件搜索工具可以“绕过正式 memory/session 检索链路”，形成非预期的跨会话检索能力。
