# Agent Platform SQLite Storage Design

## 目标

当前 `agent-platform` 同时使用 YAML、JSON、classpath 默认配置和 workspace 文件保存平台状态。随着模型、MCP、Skill、Agent、会话和运行记录增加，文件持久化已经暴露出几个问题：
...
agent:
  platform:
    persistence:
      mode: ${AGENT_PLATFORM_PERSISTENCE_MODE:file}
      sqlite:
